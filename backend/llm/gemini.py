...
class GeminiProvider(LLMProvider):
    ...
    def _chat_sync(
        self, system: str, history: list[dict], message: str, max_tokens: int = 2048
    ) -> str:
        contents = []
        for msg in history:
    ...
    async def generate_study_outline(self, context, title) -> dict:
        ...
        raw = await self._with_retry(
            self._generate_sync, self.STUDY_SYSTEM_PROMPT, prompt, 1024
        )
        return parse_json_response(raw)
    ...
    async def generate_study_section(
        ...
        return await self._with_retry(
            self._generate_sync, self.STUDY_SYSTEM_PROMPT, prompt, 2048
        )
    ...
    async def generate_flashcards(self, context, title) -> dict:
        ...
        raw = await self._with_retry(
            self._generate_sync, self.STUDY_SYSTEM_PROMPT, prompt, 1500
        )
        return parse_json_response(raw)
    ...
    async def generate_technical_section(
        ...
        return await self._with_retry(
            self._generate_sync, self.TECHNICAL_SYSTEM_PROMPT, prompt, 2048
        )
