...
function ChatView({ initialPapers, onNewSearch, sessions, setSessions, sidebarCollapsed, onToggleSidebar, onSessionSelect }: {
  ...
  // Follow-up suggestions opt-in toggle, persisted to localStorage.
  // Initialised to false (server-safe) then synced from localStorage after mount.
  const [suggestionsEnabled, setSuggestionsEnabled] = useState(false)
  useEffect(() => {
    setSuggestionsEnabled(getStoredBool('researchrag_suggestions_enabled', false))
  }, [])
...
export default function App() {
  ...
  // Always initialise to false (the server-safe default) so the server and
  // client render the same HTML on first paint. After mount, read the real
  // value from localStorage and update — this avoids the hydration mismatch
  // caused by useState(() => localStorage.getItem(...)) which the server
  // cannot evaluate.
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false)
  useEffect(() => {
    setSidebarCollapsed(getStoredBool('researchrag_sidebar_collapsed', false))
  }, [])
